...
def check_user_code_server_ready(host="localhost", port=50052, timeout=1.0):
    ...
    try:
        ...
        logger.debug(f"User code server health check: status={response.status}, ready={is_ready}")
        return is_ready
        
    except grpc.RpcError as e:
        logger.debug(f"User code server health check failed with gRPC error: {e.code()} - {e.details()}")
        return False
