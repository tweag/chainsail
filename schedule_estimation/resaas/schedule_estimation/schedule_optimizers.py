...
class SingleParameterScheduleOptimizer(AbstractScheduleOptimizer):
    ...
    def __init__(self, target_value, max_param, min_param, decrement,
                 optimization_quantity, param_name, max_replicas):
        '''
        Initializes a schedule optimizer.
    ...
    def optimize(self, dos, energies):
        ...
        new_param_msg = ("Added new parameter to schedule. Value: "
                         "{:.4f}, expected target value: {}")
        ...
        while not break_condition(params[-1], delta):
            ...
            est_q = self._optimization_quantity(
                dos, energies, params[-2], params[-1])
            log(new_param_msg.format(params[-1], est_q))

        if len(params) > self._max_replicas:
            params = self._squeeze_parameters(params)
            log(("Optimized schedule longer than allowed maximum number "
                 "of replicas. Schedule has been squeezed. Expect lower "
                 "acceptance rates."))
        log(("Schedule optimization completed. Length of new schedule: "
             f"{len(params)}"))
