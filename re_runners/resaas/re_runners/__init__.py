...
class MPIRERunner(AbstractRERunner):
    def __init__(self, hostsfile: str):
    ...
    def run_sampling(self, storage):
        ...
        mpirun_args = ["--hostsfile", self.hostsfile, "--oversubscribe", "-n", str(n_replicas + 1)]
        run_script_args = [cfg_path, "local"]
        check_output(
            ["mpirun"] + mpirun_args + ["python", get_run_script_path()] + run_script_args
        )
